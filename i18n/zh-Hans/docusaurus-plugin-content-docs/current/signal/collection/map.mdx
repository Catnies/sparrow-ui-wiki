...
<SignalStepDemo
  ...
  items={(s) => ({
    L: {icon: 'gold_ingot', name: `领先：${s.leader}`, nameColor: '#ffaa00'},
    A: {icon: 'diamond_sword', name: `Alice：${s.a} 杀`, count: Math.max(1, s.a)},
    B: {icon: 'netherite_sword', name: `Bob：${s.b} 杀`, count: Math.max(1, s.b)},
  })}
  steps={[
    {lines: '1-6', state: {kills: '{}', leader: '暂无', a: 0, b: 0}, note: '开局，还没有人击杀。'},
    {lines: '8', state: {kills: '{Alice=1}', leader: 'Alice（1 杀）', a: 1, b: 0}, notified: ['kills', 'leader'], refresh: ['L', 'A']},
    {lines: '9', state: {kills: '{Alice=1, Bob=1}', leader: 'Alice（1 杀）', a: 1, b: 1}, notified: ['kills', 'leader'], refresh: ['L', 'B'],
      note: '平手，max 取到先出现的 Alice。leader 是 map 派生的，结果没变也会通知。'},
    {lines: '10', state: {kills: '{Alice=1, Bob=2}', leader: 'Bob（2 杀）', a: 1, b: 2}, notified: ['kills', 'leader'], refresh: ['L', 'B'], note: 'Bob 反超。'},
    {lines: '12-13', state: {kills: '{Alice=1, Bob=2}', leader: 'Bob（2 杀）', a: 1, b: 2}, output: ['2', 'Bob（2 杀）']},
  ]}
/>

如果每个显示位置只关心一个 key，比如每个玩家头像下只显示他自己的击杀数，那么 Alice 的变化也会让 Bob 的显示刷新。这种情况改用 [KeyedSignal](/signal/keyed/basics)，每个 key 单独通知。
...
`MutableMapSignal` 提供 `batch`，用法和 [ListSignal](/signal/collection/list#用-batch-合并一批修改) 相同。钩子的名字略有不同。
...
<NextStep
  ...
  title="只读视图与在线玩家"
  description="对外只提供读取，以及使用 Sparrow UI 内置的在线玩家列表。"
/>
