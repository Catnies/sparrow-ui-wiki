...
商店显示的全服剩余库存保存在数据库里，其他子服也可能卖出商品。本服收不到每一次购买事件，所以无法只靠本地调用 `dirty()` 更新显示。

这时可以定期查询库存。`Signal.polling` 在有人订阅期间重复执行查询，最后一个订阅者离开后停止自动轮询。首次加载、占位值和异常处理与[异步加载](https://catnies.github.io/sparrow-ui-wiki/zh-Hans/signal/async.md)相同。
...
创建时会立即提交第一次查询。假设初始库存为 `20`，另一台服务器卖出两件后变成 `18`，下一次查询完成时，`stockText` 就能提供「剩余 18 件」。轮询之间读到的是最近一次完成的结果，无法保证它与数据库时刻一致；实际购买仍需业务层检查并扣减库存。
