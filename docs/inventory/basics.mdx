---
title: Putting an inventory in a menu
sidebar_label: Putting an inventory in a menu
---
...
- `VirtualInventory`: items live in memory, see [Virtual inventories](/inventory/virtual)
- `ReferencingInventory`: maps existing containers such as chests and player inventories into the menu, see [Referenced inventories](/inventory/referencing)
...
After modifying the inventory through these methods, every Window showing it updates automatically. They write straight into the inventory, bypassing access rules and ignoring the freeze below. The `try`-prefixed variants (`tryAdd`, `trySetItem`, and friends) check access rules first and may be rejected; see [Access rules and events](/inventory/rules-events).

`VirtualInventory` methods may run on any thread <ThreadBadge type="any" />. `ReferencingInventory` reads and writes the mapped container directly, so it must be called from a thread that may access that container; see [Thread requirements](/inventory/referencing#thread-requirements).
...
`addIngredient` binds the inventory to an identifier. The identifier's first occurrence links to slot 0, the second to slot 1, and so on, following the character template left to right, top to bottom. What happens when the slot count and inventory size disagree is covered in [Filling content](/pane/ingredients).
...
`occurrence` counts this slot's position within the identifier, starting at 0; the full parameter meaning is in [Filling content](/pane/ingredients). A slot number beyond the inventory's size makes `build()` throw an `IllegalStateException`.
...
<Details summary="The hintItem() implementation" previewHeight={0}>
...
<MinecraftSlotGrid
  ...
  rows={["#########", "####X####", "#########"]}
  legend={{'#': 'Pane background', X: 'inventory slot'}}
  defaultView="game"
  items={{
    '#': {icon: 'black_stained_glass_pane'},
    X: {icon: 'light', name: 'Put the item to appraise here', nameColor: '#ffff55', lore: ['Drop an item into this slot to start.']},
  }}
/>
...
<Details summary="The lockIcon() implementation" previewHeight={0}>
...
A [Pane freeze](/pane/composition) and an inventory freeze act on different objects:
...
<MinecraftSlotGrid
  ...
  legend={{C: 'quick slots', S: 'storage'}}
/>
...
`includeObscuredSlots(true)` lets hidden slots participate too. Below, [Pagination Page](/pagination/page) shows a 54-slot storage across two pages. Page splits on slot indexes, and the converter links each index to the matching inventory slot. Once the current page fills, shift-clicked items continue into the other page:
...
The storage below uses one independent inventory per page, switched with [Tab](/pagination/tab). While page one shows, page two's inventory is nowhere in the Window. Declaring both pages on the outer Pane lets shift-clicked items continue into page two once page one fills:
...
<Details summary="The pageButton() implementation" previewHeight={0}>
...
<NextStep
  ...
  description="Per-slot stack limits and iteration order, plus saving the contents."
/>
