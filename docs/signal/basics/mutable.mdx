---
title: "Mutable state: Signal.of"
sidebar_label: Mutable state Signal.of
---
...
<SignalStepDemo
  ...
  items={(s) => ({S: {icon: 'diamond_sword', name: `Arena score: ${s.score}`, count: Math.max(1, s.score)}})}
  steps={[
    {lines: '1', state: {score: 0}, note: 'Create the score, starting at 0.'},
    {lines: '3', state: {score: 3}, notified: ['score'], refresh: ['S'], note: 'Write 3 directly. The value changed, so the score item is redrawn on the next tick.'},
    {lines: '4', state: {score: 4}, notified: ['score'], refresh: ['S'], note: 'update adds one to the current 3, giving 4.'},
    {lines: '5', state: {score: 4}, quiet: ['score'], note: 'Old and new values are both 4. The write is skipped and the menu does not refresh.'},
    {lines: '7', state: {score: 4}, output: '4', note: 'Read the current value.'},
  ]}
/>
...
To read and write just one field, use [lens](/signal/derive/lens).
...
<NextStep
  ...
  description="Get notified with onDirty when a value changes, and manage how long the subscription lives."
/>
