...
<MinecraftSlotGrid
  ...
  caption="Alice 和 Bob 初次打开时都显示 1。Alice 点击一次后，她的按钮名称显示 2，Bob 仍显示 1。"
/>
...
将 `stockButton` 放进 Pane 后，业务代码调用 `stock.set("diamond", 9)`，所有查看它的人都会看到新库存。`keyOf` 也可以根据玩家计算队伍编号等业务 key，但它在挂载时求值；玩家中途切换队伍，不会仅因函数的返回值变了就自动重新绑定。需要动态切换来源时，可以使用 [`switching`](/signal/derive)。
