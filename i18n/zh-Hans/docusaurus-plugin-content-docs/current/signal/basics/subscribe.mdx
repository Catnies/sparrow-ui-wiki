...
<SignalStepDemo
  ...
  items={(s) => ({B: {icon: 'netherite_sword', name: `远古守卫者 ${s.hp} / 500`, nameColor: '#ff5555'}})}
  steps={[
    {lines: '1', state: {hp: 500, sub: '未订阅'}, note: 'Boss 出场，血量 500。'},
    {lines: '3-5', state: {hp: 500, sub: '已订阅'}, note: '订阅时不会补发当前值 500，控制台还是空的。需要初始值就自己 get() 一次。'},
    {lines: '7', state: {hp: 420, sub: '已订阅'}, notified: ['hp'], output: 'Boss 剩余血量：420', refresh: ['B'], note: '值变了，回调在写入的这个线程里立刻执行。'},
    {lines: '8', state: {hp: 420, sub: '已订阅'}, quiet: ['hp'], note: '写入相同的值，被跳过，回调不执行。'},
    {lines: '9', state: {hp: 350, sub: '已订阅'}, notified: ['hp'], output: 'Boss 剩余血量：350', refresh: ['B'], note: '回调里 get() 读到的是最新值 350。'},
    {lines: '11', state: {hp: 350, sub: '已关闭'}, note: '关闭订阅。之后的变化与这个回调无关。'},
    {lines: '12', state: {hp: 0, sub: '已关闭'}, notified: ['hp'], refresh: ['B'], note: '写入照常生效，依赖它的菜单也会刷新，只是控制台不再打印。'},
  ]}
/>

## 注意事项

:::warning[一定要保存 Subscription]
...
订阅的保留与清理还可以看 [生命周期与清理](/advanced/lifecycle)。菜单里的订阅怎样自动管理，见 [Signal 在 UI 中的使用](/signal-ui/item)。

<NextStep
  to="/signal/derive/map"
  title="map 转换"
  description="从一个 Signal 算出另一个值，例如把血量显示成血条文字。"
/>
