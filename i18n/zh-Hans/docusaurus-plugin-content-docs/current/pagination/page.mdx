...
<MinecraftSlotGrid
  title="钻石目录"
  rows={['MMMMMMMMM', 'P#######N']}
  legend={{M: '当前页内容', P: '上一页', N: '下一页'}}
  items={{
    M: {icon: 'diamond', name: '钻石', count: [1, 2, 3, 4, 5, 6, 7, 8, 9]},
    P: {icon: 'arrow', name: '上一页', nameColor: 'yellow'},
    N: {icon: 'arrow', name: '下一页', nameColor: 'yellow'},
  }}
  caption="预览为第一页，第二页显示数量为 10、11、12 的三组钻石。"
/>
...
这里的箭头始终保持原样。若需要显示页码，或在首页、末页将按钮变灰，可以用 `page()` 和 `count()` 提供的 Signal，见 [将 Signal 接入 UI](/signal/bind-ui)。

### 列表修改与刷新

`Page.of(List, pageSize)` 会复制列表。创建后再向原列表添加、删除或替换元素，分页内容不会跟着变化；列表中的 Item 对象本身仍是同一份，修改 Item 的显示内容依然有效。

`refresh()` 只对下文的异步分页生效，不会重新读取传给 `Page.of` 的普通 List。需要让分页跟随列表的增删变化时，见 [将 Signal 接入 UI](/signal/bind-ui)。

## 从数据库懒加载数据

如果你的菜单内的元素依赖数据库时，我们不可能为了开启菜单而全量加载数据库的所有数据，实际上我们只需要总页数和当前页的数据信息。
你可以用 `Page.async(...)` 创建一份异步加载且按需查询的分页菜单。玩家翻到哪一页，才加载哪一页，不必在打开菜单前查出全部元素。

我们以商品信息举例，下面每页查询 9 条来自数据库的商品。`repository` 是插件自己的数据库访问对象，`findPage(offset, limit)` 返回指定范围的商品，`count()` 返回商品总数。示例使用 Paper 的异步调度器执行这两种查询。
...
<Details summary="商品数据与数据库接口" previewHeight={0}>
...
创建 Page 时就会提交总页数查询，各页的商品则在首次读取时才查询。结果返回前，这一页暂时为空；查询完成后，Pane 自动显示商品。加载过的页会缓存下来，再翻回去会使用缓存。`Page.async` 的第三个参数是总页数，即使返回 0，库也会按一页处理。
...
在 Page 中提供的列表里是允许直接存业务数据，不必提前全部转成 Item 的，这样做的好处是只有在有玩家打开时才会创建 Item，减少不必要的对象内存消耗。给 `addIngredient` 再传一个转换函数，就能把当前页的数据转换成 `Element`。
...
如果列表中已经是 Item，使用上一节的两参数写法即可，不需要再包一层转换函数。
...
`pageSizeOf` 会在计算页数和内容位置时多次调用，同一个页码应返回固定容量。若每页容量相同，直接传整数即可。

<NextStep to="/pagination/scroll" title="滚动 Scroll" description="按行或按列移动内容，保留前后相邻的部分。" />
