...
窗口可以带业务对象，给点击处理器读。这页还会讲客户端状态确认，还有容器外点击事件中的光标数据和取消操作。
...
窗口仍被业务代码或会话持有时，附带的对象也会被持有。会话如何保留窗口，看 [会话与导航](/session/navigation)。
...
`getCursor()` 是事件派发时光标物品的副本，修改它不会替换实际光标物品。处理器返回后，库会检查取消状态；已取消就停止后续容器外点击处理，否则继续执行这次点击的物品操作。取消容器外点击不会阻止玩家按 Esc 关闭窗口，关闭控制看 [打开与关闭](/window/lifecycle)。
