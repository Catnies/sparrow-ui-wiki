...
结果为 `OrderOptions[quantity=2, giftWrap=false]`。Signal 保存的是对象引用，直接修改对象字段不会触发通知，再把同一个引用传给 `set` 通常也会被判为相同。因此这里用新对象替换旧状态。列表等集合的更新方式见 [集合状态](/signal/collections)。
...
菜单通过 `dependsOn` 等方式绑定 Signal 时，库会管理订阅，无需自己保存 `Subscription`。具体用法见 [Signal 在 UI 中的使用](/signal-ui/item)，订阅的寿命约定见 [生命周期与清理](/advanced/lifecycle)。
...
两次输出分别是 `60` 和 `120`。`total` 在被读取时才计算，并缓存结果；`quantity` 变化后，下一次读取才重新计算。上例跳过了数量为 `3` 时的读取，因此也跳过了总价 `90` 的计算。
...
在函数里读取另一个 Signal，不会自动建立对它的订阅。结果依赖多个数据源时，使用 [派生与组合](/signal/derive) 中的 `Signals.combine`。
:::
...
这次比较需要先计算结果，所以 `mapDistinct` 的函数也可能在写入上游时执行，应尽量简短。如果每次数量变化都会改变总价，直接用 `map` 即可。
...
:::warning[比较函数的约定]
`sameValue` 应满足等价关系，同一个值必须与自身相同，函数也应简短且无副作用。它只会收到两个非 `null` 值；两个 `null` 由库判为相同，只有一侧为 `null` 则判为不同。

Signal 会长期持有比较函数，不要在其中捕获 `Player`、`World` 或 `Window`。在数据源的比较规则中忽略一个字段，就意味着仅修改这个字段时，整次写入都会被跳过。
:::

<NextStep
  to="/signal/derive"
  title="派生与组合"
  description="组合多个数据源，切换依赖，并用防抖、节流和时钟控制更新。"
/>
