...
<SignalStepDemo
  ...
  `}
  title={(s) => `搜索：${s.keyword}`}
  layout={['IRRR#####']}
  signals={[{id: 'tick', label: '时间'}, {id: 'input', label: 'input'}, {id: 'keyword', label: 'keyword'}]}
  items={(s) => ({
    I: {icon: 'writable_book', name: `输入框：${s.input}`},
    R: s.keyword === 'diamond' ? {icon: 'diamond', name: '钻石'} : null,
  })}
  steps={[
    {lines: '1-3', state: {tick: '-', input: '', keyword: ''}, note: '输入框为空，没有搜索结果。'},
    {lines: '5-7', state: {tick: '-', input: '', keyword: ''}, note: '订阅搜索词。有了订阅，防抖才会安排定时任务。'},
    {lines: '9', state: {tick: 'tick 0', input: 'd', keyword: ''}, notified: ['input'], quiet: ['keyword'], refresh: ['I'],
      note: '开始等待 10 tick。keyword 仍是旧值，不搜索。'},
    ...
    {lines: '11', state: {tick: 'tick 6', input: 'diamond', keyword: ''}, notified: ['input'], quiet: ['keyword'], refresh: ['I'],
      note: '再次重新计时，最早要到 tick 16 才会通知。'},
    {lines: '12', state: {tick: 'tick 16', input: 'diamond', keyword: 'diamond'}, notified: ['keyword'], output: '搜索：diamond', refresh: ['R'],
      note: '三次输入只搜索了一次。'},
  ]}
/>
...
:::warning[一直输入就一直不通知]
玩家不停手，防抖就一直等下去。值持续变化、又想定期看到结果时，用 [throttle](/signal/time/throttle)。
:::
...
:::warning[间隔与初始化]
间隔必须大于 0，否则抛出 `IllegalArgumentException`。使用前要先完成 Sparrow UI 的 [初始化](/getting-started/installation#初始化)。
:::

<NextStep
  to="/signal/time/throttle"
  title="throttle 节流"
  description="限制刷新频率，例如建筑粘贴进度每半秒最多刷新一次。"
/>
