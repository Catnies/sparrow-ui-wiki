...
Referenced inventories do not switch threads for you. Calling `refresh()`, reading, and writing all touch the mapped container directly, so they must run on the thread that owns it:
...
Creating a mapping is exempt from this rule; you can build it together with the menu in an async task. Creation reads the container's contents once. Paper and Folia currently allow such async reads, and once the Window opens it syncs again on the viewer's thread. Some Fork server software does add access checks to certain methods, though; go by your actual runtime.
