...
The arrows in [Pagination Page](/pagination/page) always look usable. This time the previous arrow grays out on the first page, the next on the last, and a "1 / 2" style page number sits in the middle.
...
<MinecraftSlotGrid
  ...
  legend={{M: 'current page', P: 'previous', C: 'page number', N: 'next'}}
  items={{
    M: {icon: 'diamond', name: 'Diamond', count: [1, 2, 3, 4, 5, 6, 7, 8, 9]},
    P: {icon: 'gray_dye', name: 'Already on the first page'},
    C: {icon: 'paper', name: '1 / 2'},
    N: {icon: 'arrow', name: 'Next page'},
  }}
  caption="Page one initially. After flipping, the top shows the 10, 11, 12 stacks and the right arrow grays out."
/>
...
<Details summary="Two page-at-a-time sources" previewHeight={0}>
...
`Page.async` manages the per-page cache and supports `refresh()` and `prefetch()`; the flow is covered in [Database pagination](/pagination/page#lazy-loading-from-a-database).
...
<NextStep
  ...
  description="Scroll a dynamic list and keep the position readout current."
/>
