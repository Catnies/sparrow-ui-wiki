...
Both the shop's home page and its confirm page need the player's balance. To share one player's most recently queried result between them, turn the single balance source from [Async loading](./async) into a `PlayerKeyedSignal` holding results per player.
...
After a database write, call `balances.dirty(playerId)` to refresh that player, where `playerId` is the UUID the write touched. The previous result stays visible during the re-query, repeat refreshes merge into one follow-up, and executor and error handling follow the [async](./async) rules.
