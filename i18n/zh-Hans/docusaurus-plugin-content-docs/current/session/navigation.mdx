...
传入 Builder 时，构建就在调用 `navigate` 的线程执行。需要异步构建时，由插件安排任务，再把 Future 交给 `navigate`。实际打开窗口由 Sparrow UI 调度到玩家实体线程，构建时的线程要求看 [打开与关闭](/window/lifecycle)。
