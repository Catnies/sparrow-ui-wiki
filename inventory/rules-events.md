...
The event provides `player()`, `slot()`, `clickType()`, `hotbarButton()`, and `action()`. `slot()` is the inventory's own slot index; `action()` is the click's final effect, `NOTHING` when nothing changed.
