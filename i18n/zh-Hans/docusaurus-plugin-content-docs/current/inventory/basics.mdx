...
- `VirtualInventory`：物品保存在内存中，看 [虚拟容器](/inventory/virtual)
- `ReferencingInventory`：把箱子、玩家背包等已有的容器映射进菜单，看 [映射容器](/inventory/referencing)
...
通过这些方法修改容器后，显示这个容器的窗口会自动更新。它们直接写入容器，不经过访问规则，也不受下文的冻结限制。以 `try` 开头的同名方法（如 `tryAdd`、`trySetItem`）会先检查访问规则，结果可能被拒绝，看 [访问规则与事件](/inventory/rules-events)。

`VirtualInventory` 的方法可以在任意线程调用 <ThreadBadge type="any" />。`ReferencingInventory` 会直接读写被映射的容器，必须在能访问那个容器的线程调用，看 [线程要求](/inventory/referencing#线程要求)。
...
`addIngredient` 把容器绑定到标志符。标志符第 1 次出现的格子连接容器的第 0 格，第 2 次出现的格子连接第 1 格，依此类推，顺序与字符模板一致，从左到右、从上到下。格子数与容器大小不一致时的表现看 [填充内容](/pane/ingredients)。
...
`occurrence` 是这一格在标志符中的出现次序，从 0 开始，参数的完整含义看 [填充内容](/pane/ingredients)。格子序号超出容器范围时，`build()` 抛出 `IllegalStateException`。
...
背景设置在容器上，这个容器显示在哪个菜单，空格都显示同一个背景。`setBackground` 还接受 `ItemProvider`，可以在渲染时按查看者生成背景，写法看 [渲染与刷新](/item/render)。传入 `null` 清除背景，正在显示的窗口会随之更新。
