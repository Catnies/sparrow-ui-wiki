...
<MinecraftSlotGrid
  ...
  rows={['MMMMMMMMM', 'P#######N']}
  legend={{M: 'current page', P: 'previous', N: 'next'}}
  items={{
    M: {icon: 'diamond', name: 'Diamond', count: [1, 2, 3, 4, 5, 6, 7, 8, 9]},
    P: {icon: 'arrow', name: 'Previous page', nameColor: 'yellow'},
    N: {icon: 'arrow', name: 'Next page', nameColor: 'yellow'},
  }}
  caption="The preview shows page one; page two displays the three stacks with counts 10, 11, 12."
/>
...
The arrows here never change. To show the page number or gray out the buttons at either end, use the Signals from `page()` and `count()`; see [Paging buttons and page numbers](/signal-ui/page).
...
`refresh()` only applies to the async pagination below; it does not re-read a plain List handed to `Page.of`. To have pagination follow list changes, see [Paging buttons and page numbers](/signal-ui/page).
...
<Details summary="Product data and the database interface" previewHeight={0}>
...
<NextStep to="/pagination/scroll" title="Scroll" description="Move content row by row or column by column, keeping the neighboring parts visible." />
