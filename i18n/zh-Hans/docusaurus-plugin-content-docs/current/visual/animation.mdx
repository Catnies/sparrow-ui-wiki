...
`#` 没有绑定内容，同样可以用 `pane.slots("#")` 取出它的格子。循环动画要保留 `play` 返回的句柄，不需要时调用 `cancel()` 停止，看 [播放句柄](#播放句柄)。
...
- `SlotSequence.all`、`row`、`column`、`borders`、`rectangle`：按形状选取，看 [程序化布局](/pane/programmatic)
