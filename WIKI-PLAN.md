...
- 1.3 `getting-started/concepts`（核心概念与层级）已完成，连带做出 `<ConceptMap>`
...
下一页是第 2 组「物品 Item」的 2.1 `item/create`。
...
6. **文案去 AI 味。** 交付前扫一遍：中文破折号（——）一律改成逗号或句号；不写「不是 A 而是 B」这种假转折（多半前半句在打一个没人提过的靶子，直接说后半句）；不写「本质上」「说白了」「值得注意的是」「综上所述」这类套话；连续三项的排比能压成两项就压。
   代码块前的冒号是正常技术写法，不在此列。
7. **入门组（第 1 组）控制信息密度。** 概念页只讲有哪几层、各管什么，细节留给对应章节。入门阶段铺太细反而抬高理解成本。
8. **给约束要写前提。** 「不能在组件里存玩家状态」这类限制只在特定用法下成立（这里是确实把同一个组件复用给多名玩家），写的时候必须先说清默认做法是什么——本库的默认做法是**每次打开建一份**，构建成本很低。不写前提会让读者以为默认路径上就有这个坑。
...
| `<ConceptMap>` | **包含关系**图：一层套一层的盒子，点任意一层出职责与跳转链接。最内层画成一排格子 | 1.3，以及任何需要讲清楚谁装着谁的地方 | `layers=[{id,label,note,desc,to}]`（从外到内）`aside` `initial` | ✅ |
...
| 组件 | 章节 | 要做到什么 |
|---|---|---|
| ~~核心概念层级交互图~~ ✅ 做成了 `<ConceptMap>` | 1.3 | 已完成，并已提升为通用组件登记在 4.1。概念数据由页面用 props 传入，文案留在译文树里。**画的是包含关系（盒子套盒子），不是流程线**——流程线看着像调用顺序，会误导 |
