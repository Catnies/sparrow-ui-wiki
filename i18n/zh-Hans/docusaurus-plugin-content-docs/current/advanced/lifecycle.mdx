...
结束观察时调用 `watch.close()`。Signal 弱持有订阅节点，凭证被回收后，订阅可能随之消失。所以不能只调用 `onDirty(...)` 就丢掉返回值，否则监听可能在运行一段时间后停止。
...
`Item.dependsOn`、Pane 的列表绑定和 `visual().bind` 也由对应 UI 对象管理，不用仅为保持有效而额外保存凭证。不过，Pane 的绑定可能在没有窗口查看时仍然活动。共享 Pane 绑定了轮询数据时，关闭其中一扇窗口不一定会停止查询。
