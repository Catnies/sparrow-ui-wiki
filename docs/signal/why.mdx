...
<TabItem value="without" label="Without a Signal" default>
...
<TabItem value="with" label="With a Signal">
...
<SignalFlowDemo
  title="Shop"
  sources={[
    {id: 'quantity', label: 'Quantity', value: 1},
    {id: 'balance', label: 'Balance', value: 100, format: (value) => `${value} coins`},
    {id: 'stock', label: 'Stock', value: 5},
  ]}
  derived={[
    {id: 'total', label: 'Total', deps: ['quantity'], compute: (values) => values.quantity * 30, format: (value) => `${value} coins`},
    {id: 'affordable', label: 'Affordable', deps: ['quantity', 'balance', 'stock'],
      compute: (values) => values.quantity <= values.stock && values.balance >= values.quantity * 30,
      format: (value) => (value ? 'yes' : 'no')},
  ]}
  consumers={[
    {id: 'quantityItem', label: 'Quantity', deps: ['quantity'], slot: 1,
      item: (values) => ({icon: 'paper', name: `Quantity: ${values.quantity}`, count: values.quantity})},
    {id: 'totalItem', label: 'Total', deps: ['total'], slot: 3,
      item: (values) => ({icon: 'gold_ingot', name: `Total: ${values.total} coins`})},
    {id: 'confirmItem', label: 'Buy button', deps: ['affordable'], slot: 5,
      item: (values) => (values.affordable
        ? {icon: 'lime_dye', name: 'Buy', nameColor: '#55ff55'}
        : {icon: 'gray_dye', name: 'Cannot buy', nameColor: '#aaaaaa'})},
    {id: 'balanceItem', label: 'Balance', deps: ['balance'], slot: 7,
      item: (values) => ({icon: 'emerald', name: `Balance: ${values.balance} coins`})},
  ]}
  actions={[
    {label: 'Quantity +1', apply: (values) => ({quantity: Math.min(9, values.quantity + 1)})},
    {label: 'Quantity −1', apply: (values) => ({quantity: Math.max(1, values.quantity - 1)})},
    {label: 'Balance +50', apply: (values) => ({balance: values.balance + 50})},
    {label: 'Balance −50', apply: (values) => ({balance: Math.max(0, values.balance - 50)})},
    {label: 'Stock −1', apply: (values) => ({stock: Math.max(0, values.stock - 1)})},
  ]}
/>
...
<NextStep
  ...
  title="Mutable state"
  description="Create a state you can read and write with Signal.of."
/>
