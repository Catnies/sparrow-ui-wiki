...
<MinecraftSlotGrid
  ...
  rows={['####Q####']}
  legend={{Q: "current player's quantity"}}
  items={{Q: {icon: 'diamond', name: 'Quantity: 1'}}}
  caption="Alice and Bob both start at 1. After Alice clicks once, her name shows 2 while Bob still shows 1."
/>
...
Once `stockButton` sits in a Pane, a business call to `stock.set("diamond", 9)` shows everyone the new stock. `keyOf` can also compute business keys such as team ids from the player, but it is evaluated at mount time; a player switching teams mid-session does not rebind just because the function would now return differently. When the source should switch dynamically, express that with [`switching`](/signal/derive) first.

<NextStep
  to="/signal-ui/list"
  title="List contents"
  description="Map a product list that keeps growing and shrinking onto menu slots."
/>
