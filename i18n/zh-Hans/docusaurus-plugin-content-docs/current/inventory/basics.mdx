...
- `ReferencingInventory`：读写箱子、玩家背包等已有的容器，见 [引用外部容器](/inventory/referencing)
...
`VirtualInventory` 的方法可以在任意线程调用 <ThreadBadge type="any" />。`ReferencingInventory` 会直接读写被引用的容器，必须在能访问那个容器的线程调用。
...
`OperationCategory` 位于 `net.momirealms.sparrow.ui.inventory.operation`，优先级按操作分成两类：
