---
title: Submenus and going back
sidebar_label: Submenus and going back
---
...
With a Builder, building runs on the thread calling `navigate`. For async building, schedule the task yourself and hand the Future to `navigate`. The actual open is scheduled onto the player's entity thread; the build-time thread requirements are in [Opening and closing](/window/lifecycle).
...
If the next Window needs business data such as product info, pass it at build time with `setData`, see [Window data](/window/data). Joining a session does not forward that data automatically.
...
Sessions default to the stack structure, returning level by level in entry order. The tree structure instead lets each Window return to its own parent; the differences are in [The three session structures](/session/structures).
...
A plugin calling `close()` still closes the current Window and ends the session; use `back()` or `backOrClose()` when you want a return. If the Window has `setCloseable(false)`, player close attempts are blocked, see [Opening and closing](/window/lifecycle).
...
<NextStep to="/session/structures" title="The three session structures" description="Compare how each structure returns, and whether exited Windows are kept." />
