...
When a menu binds a Signal through `dependsOn` and friends, the library owns the subscription and you keep no `Subscription`. The binding itself is covered in [Signals in the UI](/signal-ui/item), and subscription lifetime rules in [Lifecycle and cleanup](/advanced/lifecycle).
...
Reading another Signal inside the function does not subscribe to it. When the result depends on several sources, use `Signals.combine` from [Deriving and combining](/signal/derive).
:::
...
<NextStep
  ...
  description="Combine several sources, switch dependencies, and pace updates with debounce, throttle, and clocks."
/>
