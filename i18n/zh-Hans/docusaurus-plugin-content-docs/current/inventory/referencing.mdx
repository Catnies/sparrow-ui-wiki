...
`ReferencingInventory` 把箱子、玩家背包等已有的容器映射进菜单。它自己不保存物品，读取时读的是被映射容器的当前内容，写入时直接写进被映射的容器。
...
这个方法在玩家右键木桶的事件处理器等能访问木桶的线程中调用，原因见 [线程要求](#线程要求)。窗口下方默认显示玩家背包，玩家可以在木桶与背包之间 Shift 转移、双击收集。
...
其他插件自己实现的 `Inventory` 通过 Bukkit 的 `getItem`、`setItem` 读写，不会自动退役。退役的含义见 [退役](#退役)。

同一个位置被映射多次时，例如大箱子和其中一半，库能认出它们是同一格。一个窗口同时显示这两份映射时，Shift 转移和双击收集不会把同一格处理两次。

映射容器的每格上限来自被映射的容器，遍历顺序也由库决定，没有 `setMaxStackSize` 和 `setIterationOrder`。访问规则、背景、冻结、优先级和事件订阅的用法与虚拟容器相同。`referencedInventory()` 返回被映射的 Bukkit 容器，退役后返回 `null`。
...
物品通过 Shift 转入这个映射时，和原版一样按反向顺序放入：从快捷栏的最后一格开始，经过快捷栏，再到主背包。窗口默认的下部 Pane 也是这样映射查看者自己的背包，可以用 `window.defaultLowerInventory()` 取得，见 [窗口布局](/window/layout)。

:::warning[Folia 上不能这样映射其他玩家]
查看者和 `target` 在 Folia 上由不同的线程管理，窗口在查看者的线程上读写 `target` 的背包，不满足 [线程要求](#线程要求)。上例只适用于 Paper。
:::
...
窗口显示映射容器期间，每 tick 在查看者的线程上调用一次 `refresh()`：发现改动后更新窗口，并派发来源为 `UpdateReason.External` 的更新事件。外部改动已经发生，所以不经过访问规则，也不能取消，事件的用法见 [访问规则与事件](/inventory/rules-events)。通过 [`linkInventory`](/inventory/basics#没有显示的容器) 声明的映射容器同样会刷新。每次写入前，映射容器也会先刷新一次，按最新内容计算。
...
|---|---|
| `size()` | 格子数，创建后不能变化 |
| `read(slot)` | 读取一格，空格返回 `null`。可以直接返回内部的物品，库只读取，不会修改或保存它 |
| `write(slot, item)` | 写入一格，`null` 表示清空。传入的物品交给存储，库之后不会再修改它 |
...
这些方法在映射容器被访问的线程上调用，同样遵守 [线程要求](#线程要求)。
...
<NextStep
  ...
  title="访问规则与事件"
  description="限制玩家能放入、取出什么，并在容器内容变化时执行自己的逻辑。"
/>
