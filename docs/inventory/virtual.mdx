...
sidebar_label: Virtual inventories
---
...
Every virtual inventory has a UUID, random by default, written out on save. To supply your own, use `new VirtualInventory(uuid, size)` or `new VirtualInventory(uuid, items)`; see [Saving and loading](#saving-and-loading).

Once an inventory is bound to a Pane, Sparrow UI handles the player's actions, subject to the [access rules](/inventory/rules-events) and the [freeze](/inventory/basics#freezing-inventories).
...
Stack limits only cap quantities. Restricting slots to gems specifically is the job of [access rules](/inventory/rules-events).
...
<MinecraftSlotGrid
  title="Display case"
  rows={["SSSSSSSSS"]}
  legend={{S: 'display case'}}
  defaultView="game"
  items={{S: {icon: 'paper', name: 'Fill order', count: [8, 6, 4, 2, 1, 3, 5, 7, 9]}}}
  caption="Counts show the fill sequence; the middle slot, unnumbered here, fills first."
/>
...
Iteration order decides which slot goes first inside one inventory. With several inventories in a Window, which inventory goes first is decided by [priority](/inventory/basics#priority).
...
<NextStep
  ...
  description="Plug player inventories, chests, and other existing containers into the menu."
/>
