...
This page walks that flow with `Signal.async`, then shows how to re-query after granting a reward. The wiring for item names and window display lives in [Binding to the UI](/signal-ui/item); here we watch the results from the console.
...
The example spells out both the first read and the subscription because `onDirty` does not replay the current value. The console subscription here only observes changes; call `subscription.close()` when done. How real menus bind is covered in [Binding to the UI](/signal-ui/item).
...
New and old balances are compared with `Objects.equals` by default, so an unchanged result notifies nobody. For custom equality, pass `sameValue` as the last argument of `Signal.async`; the rules are in [Signal basics](./basics).
...
The current round's result still publishes even when a follow-up is booked. `dirty()` never cancels the running query or throws its result away as stale. When queries switch by search term, combine with [`switching`](./derive) from the previous page so different terms never write into the same state.
...
<NextStep
  to="/signal/polling"
  title="Polling"
  description="When other services can change the data, re-query on a schedule while someone is watching."
/>
