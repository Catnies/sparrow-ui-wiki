...
结果为 `OrderOptions[quantity=2, giftWrap=false]`。Signal 保存的是对象引用，直接修改对象字段不会触发通知，再把同一个引用传给 `set` 通常也会被判为相同。所以这里用新对象替换旧状态。列表等集合的更新方式看 [集合状态](/signal/collections)。
...
菜单通过 `dependsOn` 等方式绑定 Signal 时，库会管理订阅，不用自己保存 `Subscription`。具体用法看 [Signal 在 UI 中的使用](/signal-ui/item)，订阅的寿命约定看 [生命周期与清理](/advanced/lifecycle)。
