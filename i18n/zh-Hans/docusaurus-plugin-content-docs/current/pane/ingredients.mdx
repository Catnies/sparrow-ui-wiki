...
## 标志共享物品：Item、ItemStack、ItemProvider

绑定 `Item` 时，标志符占的每一格都显示这个 Item。`ItemStack` 和 `ItemProvider` 是简写，库会用 `Item.simple(...)` 替你包装成 Item：
...
简写得到的 Item 只负责显示，点击没有任何反应。需要点击行为、守卫或刷新规则时，用 `Item.builder()` 创建完整的 Item，见 [创建物品](/item/create)。
...
大多数时候，共用一个 Item 和每格一个 Item 看不出区别。Item 自己带有状态时就不一样了，比如下面这个记录点击次数的按钮：
...
`VirtualInventory` 是库提供的内存容器，位于 `net.momirealms.sparrow.ui.inventory`。容器的规则、事件等用法见 [把容器放进菜单](/inventory/basics)。

:::warning[格子数与容器大小保持一致]
格子比容器多时会从头循环，同一个容器格出现在两个位置；格子比容器少时，多出来的容器格不显示。
:::

:::warning[新建的容器随菜单一起丢失]
在打开菜单的方法里新建 `VirtualInventory` 时，菜单关闭后这个容器就不再被引用，玩家放进去的物品也随之丢失。

需要保存物品时，在关闭前序列化容器内容，见 [虚拟容器](/inventory/virtual)；需要多名玩家共用一个容器时，让菜单引用同一个实例。
:::
...
标志符占据的区域比子 Pane 大时，多出的格子找不到对应位置，`build()` 抛出 `IllegalStateException`。区域比子 Pane 小时只显示子 Pane 左上角的一部分，想显示其他部分，或了解嵌套后的背景与点击规则，见 [背景、冻结与嵌套](/pane/composition)。
:::

:::info[翻页、滚动、标签与 Signal]
下面几种内容同样用 `addIngredient` 绑定，它们会在数据变化时更新格子，分别在后面的章节介绍：

- `Page`：按页显示一份列表，见 [翻页 Page](/pagination/page)
- `Scroll`：按行或按列滚动显示一份列表，见 [滚动与标签](/pagination/scroll-tab)
- `Tab`：在几块 Pane 之间切换，见 [滚动与标签](/pagination/scroll-tab)
- `Signal` 列表：列表数据变化后自动更新格子，见 [把 Signal 接回菜单](/signal/bind-ui)
:::
