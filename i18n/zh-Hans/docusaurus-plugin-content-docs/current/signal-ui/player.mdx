...
下面的 `quantities` 和 `amountButton` 可以在商店服务中创建一次，让多个窗口复用。每次打开菜单时，把这个按钮放进对应 Pane 即可。
...
<MinecraftSlotGrid
  ...
  caption="Alice 和 Bob 初次打开时都显示 1。Alice 点击一次后，她的名称显示 2，Bob 仍显示 1。"
/>
...
将 `stockButton` 放进 Pane 后，业务代码调用 `stock.set("diamond", 9)`，所有查看它的人都会看到新库存。`keyOf` 也可以根据玩家计算队伍编号等业务 key，但它在挂载时求值；玩家中途切换队伍，不会仅因函数的返回值变了就自动重新绑定。需要动态切换来源时，先用 [`switching`](/signal/derive) 表达这种变化。

<NextStep
  to="/signal-ui/list"
  title="列表内容"
  description="把不断增删的商品列表映射到菜单格子。"
/>
