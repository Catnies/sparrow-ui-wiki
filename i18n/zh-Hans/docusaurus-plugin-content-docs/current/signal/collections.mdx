...
本页用这个商店的商品列表、选中项和价格表介绍集合 Signal。它们保留 `List`、`Set`、`Map` 的操作方式，修改后会通知依赖者。下面先用控制台观察数据变化，接入菜单的方式见[绑定到 UI](/signal-ui/list)。

## 直接修改列表、集合和映射

先处理上架与下架。`products` 保存当前出售的商品名称，`count` 是菜单需要显示的商品种类数。它由列表大小计算，后面只修改商品列表即可。
...
输出从「苹果 20 金币」变成「苹果 25 金币」。`prices.get("apple")` 可以直接读取价格，无参数的 `prices.get()` 则返回整张映射。这里任意商品的价格变化都会让映射的依赖失效；需要按商品分别订阅时，见[分区状态](./partitions)。
...
下面用苹果、面包两条记录代替配置加载的结果。包装后再加入钻石，派生出的商品编号列表应保持这个顺序。
...
回调只输出一次 `[苹果, 面包, 钻石]`。`SetSignal` 和 `MapSignal` 也提供 `batch`。嵌套调用时，通知延后到最外层结束；期间没有变更，就不会通知。

:::warning[batch 不提供回滚或隔离]
...
**钩子返回的 `Subscription` 必须保存，结束使用时再关闭。** 集合只弱持有钩子，丢弃凭证后，钩子可能被回收。它们在修改集合的线程同步执行，应在集合交给其他代码使用前配置好，避免在钩子中反过来修改同一个集合。
...
<NextStep
  to="/signal/partitions"
  title="分区状态"
  description="按商品或玩家保存独立状态，分别管理读取、订阅和缓存。"
/>
