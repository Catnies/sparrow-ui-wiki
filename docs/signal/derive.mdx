---
title: Deriving and combining
sidebar_label: Deriving and combining
---
...
<SignalFlowDemo
  ...
  sources={[
    {id: 'quantity', label: 'Quantity', value: 2},
    {id: 'price', label: 'Unit price', value: 30},
    {id: 'coins', label: 'Balance', value: 100},
  ]}
  derived={[
    {id: 'total', label: 'Total', deps: ['quantity', 'price'], compute: (v) => v.quantity * v.price},
    {id: 'affordable', label: 'Affordable', deps: ['quantity', 'price', 'coins'],
      compute: (v) => v.quantity * v.price <= v.coins, format: (v) => v ? 'yes' : 'no'},
  ]}
  consumers={[
    {id: 'totalItem', label: 'Total item', deps: ['total'], slot: 3,
      item: (v) => ({icon: 'gold_ingot', name: `Total: ${v.total} coins`})},
    {id: 'confirmItem', label: 'Buy button', deps: ['affordable'], slot: 5,
      item: (v) => v.affordable ? {icon: 'lime_dye', name: 'Buy'} : {icon: 'gray_dye', name: 'Not enough coins'}},
  ]}
  actions={[
    {label: 'Quantity +1', apply: (v) => ({quantity: v.quantity + 1})},
    {label: 'Price +10', apply: (v) => ({price: v.price + 10})},
    {label: 'Balance +50', apply: (v) => ({coins: v.coins + 50})},
  ]}
/>
...
With a `KeyedSignal` already in hand, `Signals.switching(source, category)` reads the partition matching the current key. Creating and managing partitions is covered in [Partition state](/signal/partitions).
...
When the list holds business objects, the second argument reads that object's Signal, for instance `Warehouse::stock`. Return a Signal the member already owns, and keep the iteration order of the member collection stable. A plain `MutableSignal<List<T>>` publishes a new list via `set`; `ListSignal` and `SetSignal` can serve as member sources directly, see [Collection state](/signal/collections).
...
<NextStep
  to="/signal/async"
  title="Async loading"
  description="Query the balance when the shop opens, and re-read results after rewards are granted."
/>
