...
sidebar_label: Collection state
---
...
The output moves from "Apple 20 coins" to "Apple 25 coins". `prices.get("apple")` reads one price directly, while the no-argument `prices.get()` returns the whole map. Any product's price change invalidates the map's dependents; to subscribe per product, see [Partition state](./partitions).
...
<NextStep
  ...
  description="Keep independent state per product or player, each with its own reads, subscriptions, and cache."
/>
